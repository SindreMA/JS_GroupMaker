...
module.exports = {
    ...
    CreateGroupItem(templateid, title,desscription,level, mapid, userId,channelId, messageId, maxTanks, maxHealers,maxdamages) {
        return new Promise((resolve, reject) => {
            pool.query(`INSERT INTO public."Groups" (template, "timestamp", title, description, level, map, admin, channel, message, maxtanks, maxhealers, maxdamages) VALUES (${templateid},${ac.getUnixTimestamp()},'${title}','${desscription}','${level}',${mapid},${userId},${channelId},${messageId},${maxTanks},${maxHealers},${maxdamages});`, (err,res) => {
                if (!err && res) {
                    resolve()
                } else if (err) {
                    reject(err)
    ...
    DeleteGroupItem(id) {
        return new Promise((resolve, reject) => {
            pool.query(`UPDATE public."Groups" SET disabled=true WHERE id=${id};`, (err,res) => {
                if (!err && res) {
                    resolve(res.rows)
                } else if (err) {
                    reject(err)
    ...
    GetGroupItem(messageId ) {
        return new Promise((resolve, reject) => {
            pool.query(`SELECT * FROM public."Groups" where message = ${messageId};`, (err,res) => {
                if (!err && res) {
                    resolve(res.rows)
                } else if (err) {
                    reject(err)
